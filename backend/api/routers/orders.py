...
from api.email.email_notifications import send_order_confirmation_from_order
...
async def create_order(
    ...
    try:
        ...
        items_data = json.loads(new_order.items_json.replace("'", '"'))
        
        # Send order confirmation email
        email_sent = send_order_confirmation_from_order(new_order, items_data)
        
        if email_sent:
            print(f"✅ Order confirmation email sent to {new_order.customer_email}")
        else:
            print(f"⚠️  Order confirmation email failed for {new_order.customer_email}")
    except Exception as e:
        print(f"⚠️  Error sending order confirmation email: {str(e)}")
